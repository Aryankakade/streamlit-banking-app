...
from flask_cors import CORS
import pymysql
import os  # ✅ Secure Environment Variables

app = Flask(__name__)
CORS(app)  # ✅ Allow CORS for Streamlit & Frontend

# ✅ Secure Database Connection
def get_db_connection():
    try:
        conn = pymysql.connect(
            host=os.getenv("DB_HOST"),  # ✅ Remote MySQL Host (e.g., AWS RDS, ClearDB)
            user=os.getenv("DB_USER"),  # ✅ Fetch from Render Environment Variables
...
@app.route("/")
def home():
    return jsonify({"message": "Banking API Running!", "status": "success"}), 200

# ✅ Fetch Table Data (Secure)
@app.route("/all_data/<table_name>", methods=["GET"])
def get_table_data(table_name):
    try:
        conn = get_db_connection()
...
def get_kpis():
    try:
        ...
        print(f"[ERROR] KPI Fetch Error: {e}")
        return jsonify({"error": str(e)}), 500

# ✅ Execute SQL Query (Secure)
@app.route("/execute_query", methods=["POST"])
def execute_query():
    try:
        data = request.get_json()
...
if __name__ == "__main__":
    print("[INFO] Flask API Running on Port 5000...")
